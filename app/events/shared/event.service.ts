...
export class EventService {
  ...
  saveEvent(event):Observable<IEvent> {
    let headers = new Headers({'Content-Type':'application/json'}) // telling what data is coming from front end to the back
  ...
  searchSessions(searchTerm:string){ // called by navbar.component.ts
    ...
    // simulating a webservice request for data
    var emitter = new EventEmitter(true); // true for async data
    setTimeout(() =>{ // () means no params are passed in to function
